...
import { handleSwapTokens } from "@/lib/handlers"
...
export default function SwapPage() {
  ...
  const handleSwapAction = async (amountSwapIn: string, amountMinSwapOut: string, inputToken: string ) => {
    setError(null);
    ...
    try {
      console.log("Swapping Tokens:", { tokenA, tokenB,inputToken, amountSwapIn, amountMinSwapOut });
      const txSig = await handleSwapTokens(
        tokenA,
        tokenB,
        inputToken,
        Number(amountSwapIn),
        Number(amountMinSwapOut),
        wallet
      );
